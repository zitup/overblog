...
### import
  ...
  1. 引入只读的**实时绑定**的模块，但是不会立即执行，而是只生成一个引用。（实时绑定指导出的模块改变时，引入的值也会更新。）
  2. 引入的模块默认在**严格模式**下运行。
...
1. 循环加载
  ...
  - CommonJS
    ...
    一旦出现循环加载，就只输出已经执行的部分，剩余部分不会输出

  - ESM

    ES6 不会关心是否发生了"循环加载"，只是生成一个指向被加载模块的引用，只要在真正取值的时候能够取到值，就可以正确执行

    > http://www.ruanyifeng.com/blog/2015/11/circular-dependency.html

2. ES modules 浏览器支持情况？现在编译成了什么样子？Vite？
3. 模块化与工程化：webpack。webpack 同时支持 CommonJS、AMD 和 ESM 三种模块化规范的打包。根据不同规范 webpack 会将模块处理成不同的产物。看看产物
