  ...
  HTTP/2 正式发表于 2015 年。
   ...
   1. HTTP/1.1
       ...
       2. HTTP/1.1 缺点
            ...
            **TCP 连接数限制**
            ...
            浏览器对 TCP 连接数有限制，请求较多时会阻塞。Chrome 为 6，普遍为 6～8。
   ...
   2. HTTP/2
       ...
       - 二进制分帧
          ...
          流是存在于连接中的一个虚拟通道。流可以承载双向消息，每个流都有一个唯一的整数ID。

          HTTP/2 将请求和响应数据分割为更小的帧，并且它们采用二进制编码，传输更高效。

          **HTTP/2 中，同域名下所有通信都在单个连接上完成，该连接可以承载任意数量的双向数据流。**每个数据流都以消息的形式发送，而消息又由一个或多个帧组成。多个帧之间可以乱序发送，根据帧首部的流标识可以重新组装。

       - 多路复用

          HTTP/2 允许在单个 TCP 连接上并行地处理多个请求和响应，解决了 HTTP/1.1 下，TPC 连接队头阻塞和浏览器对 TCP 连接数的限制问题。
   ...
   3. HTTP/3
      ...
      HTTP 跑在 QUIC 协议上。QUIC 协议基于 UDP 协议，抛开了 TCP 协议。因为 TCP 连接需要经过三次握手，如果再加上 TLS，还需要经过多次协商，才能正式传输数据。而 UDP 是无连接的，不需要握手和挥手，效率更高。QUIC 主要有以下特点：

       - 实现了类似 TCP 拥塞控制、传输可靠性的功能
       - 快速握手功能
       - 集成了 TLS 加密功能
       - 多路复用。在 HTTP/2 中，如果发生丢包，由于 TCP 的“丢包重传”机制，丢失的包必须等待重新传输确认，导致整个 TCP 都要开始等待重传，就会阻塞该 TCP 连接中的所有请求。QUIC 实现了数据流的单独传输，就解决了丢包时的阻塞问题。
...
https://www.jiqizhixin.com/articles/2020-06-01-12
