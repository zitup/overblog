...
闭包由函数和创建函数时的词法环境组成。在 JS 中，每创建一个函数，就创建了闭包。
...
### 闭包的本质
函数仍然记录着它创建时所在的词法环境的引用，包含其中的变量。词法环境是用来定义标识符与变量和函数的关联。可以简单理解为在当前词法环境内创建的变量和值之间的一个映射。

对词法环境的引用赋予了函数可以访问外部变量的能力，而且无论它在哪或什么时候执行。可以理解为将外部的值捕获(capture)了。
